...
connection::connection(SOCKET fd, int32 id)
...
int32	connection::handle_read(){
	...
	while(true){
		...
		int32	len = check_pack_len();
		while(len > 0){
			write_log(tag,EF_LOG_LEVEL_ALL,
				"con:%p, id:%u, fd:%d, buf_size:%d, "
				"pack_len:%d, handle_pack!", 
				this , m_id, m_fd, m_buf.size(), len);
			std::string	req;
			read_buf((char*)req.data(), len);	
			ret = handle_pack(req);
			if(ret < 0){
				return	ret;
			}
...
int32	connection::send_message(const std::string &msg){
	int32	ret = 0;
	m_send_queue_size += (int32)msg.size();
...
int32	connection::do_send(){
	...
	while(m_msgs.begin() != m_msgs.end()){
		...
		int32	len = msg.length() - m_cur_msg_start;
